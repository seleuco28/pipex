...
/*   Updated: 2023/02/14 12:46:09 by alvelazq         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char *argv[], char *envp[])
{
	...
	if (pipe(pipex.tube) < 0) // aqui es la unica vez en todo el programa en que hago pipe. Pipe le asigna un FD a cada end[]
		msg_error(ERR_PIPE);
	...
	// if (pipex.pid < 0)
	//	return(perror("EL process id es negativo."))
	if (pipex.pid == 0) //los id de los child process son siempre CERO
